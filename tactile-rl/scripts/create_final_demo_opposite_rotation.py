#!/usr/bin/env python3
"""
Create final demonstration with joint 6 rotated -45 degrees (opposite direction).
"""
...
class FinalDemoOppositeRotation:
    def __init__(self):
        # Load model
        ...
        # Sequence with joint 6 = -0.785 rad (-45 degrees)
        self.sequence = [
            # Phase, duration, [j0, j1, j2, j3, j4, j5, j6], gripper, description
            # Joint 6 is now -0.785 (negative 45 degrees)
            ("home", 60, [0, -0.5, 0, -2.0, 0, 1.57, -0.785], 255, "Home - J6 Rotated -45°"),
    ...
    def run(self):
        """Create the video."""
        print("Creating FINAL DEMO with OPPOSITE ROTATION...")
        print("- Joint 6 at -0.785 rad (-45°) - OPPOSITE DIRECTION")
        print("- Safe approach to blocks at X=0.05m")
        print("=" * 70)
        ...
        for phase_idx, (phase_name, duration, target_config, gripper_cmd, description) in enumerate(self.sequence):
            ...
            for frame in range(duration):
                ...
                # Overlays
                cv2.putText(frame_img, "PANDA ROBOT - FINAL DEMO (J6 = -45°)", (40, 60),
                ...
                # Joint 6 info - HIGHLIGHT THE OPPOSITE ROTATION
                joint_config = [self.data.qpos[14 + i] for i in range(7)]
                cv2.putText(frame_img, f"Joint 6: {joint_config[6]:+.3f} rad ({np.degrees(joint_config[6]):+.1f}°) ← OPPOSITE ROTATION",
                           (40, y + 105), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 100), 2)
                
                # Gripper orientation - show all axes
                cv2.putText(frame_img, "GRIPPER ORIENTATION:", (40, y + 140),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                cv2.putText(frame_img, f"  X (Right):   [{gripper_x[0]:+.2f}, {gripper_x[1]:+.2f}, {gripper_x[2]:+.2f}]",
                           (40, y + 165), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 200, 200), 2)
                cv2.putText(frame_img, f"  Y (Up):      [{gripper_y[0]:+.2f}, {gripper_y[1]:+.2f}, {gripper_y[2]:+.2f}]",
                           (40, y + 185), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200, 255, 200), 2)
                cv2.putText(frame_img, f"  Z (Forward): [{gripper_z[0]:+.2f}, {gripper_z[1]:+.2f}, {gripper_z[2]:+.2f}]",
                           (40, y + 205), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200, 200, 255), 2)
                
                # Table clearance
                clear_color = (0, 255, 0) if table_clearance > 0.02 else (255, 0, 0)
                cv2.putText(frame_img, f"Table Clearance: {table_clearance:.3f}m",
                           (40, y + 235), cv2.FONT_HERSHEY_SIMPLEX, 0.9, clear_color, 2)
                ...
                cv2.putText(frame_img, f"Gripper: {state}",
                           (40, y + 265), cv2.FONT_HERSHEY_SIMPLEX, 0.9, state_color, 2)
        ...
        output_path = "../../videos/121pm/panda_final_opposite_rotation.mp4"
        ...
        if frames:
            ...
            print(f"\n✅ FINAL VIDEO WITH OPPOSITE ROTATION SAVED!")
            print(f"📹 Path: {output_path}")
            print(f"📊 Statistics:")
            print(f"  Joint 6: -0.785 rad (-45.0°) - OPPOSITE DIRECTION")
            print(f"  Minimum approach distance: {min_approach_dist:.3f}m")
            print(f"  Minimum table clearance: {min_clearance:.3f}m")

if __name__ == "__main__":
    creator = FinalDemoOppositeRotation()
    creator.run()